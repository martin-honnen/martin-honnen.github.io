...
<xsl:variable name="v:title-groups" as="element()+"
  ...
  <title xpath="self::db:figure[parent::db:formalgroup]
         ...
         group="{if (f:is-true($formal-object-numbers))
                 then 'subfigure-title-numbered'
                 else 'subfigure-title-unnumbered'}"/>
...
<xsl:template match="*" mode="mp:compute-headline-label" as="item()*">
  ...
  <!--
  <xsl:message select="$template"/>
  -->

  <xsl:choose>
    <xsl:when test="f:is-true($use-label-numeration) and @label">
      <xsl:value-of select="@label"/>
    </xsl:when>
    <xsl:when test="$template/lt:label">
