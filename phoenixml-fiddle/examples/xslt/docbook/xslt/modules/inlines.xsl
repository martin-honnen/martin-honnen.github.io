...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:oxy="http://docbook.org/ns/docbook/oxygen"
...
<xsl:template match="db:citetitle">
  <xsl:call-template name="t:inline">
    ...
    <xsl:with-param name="class" select="@pubwork"/>
  </xsl:call-template>
</xsl:template>
...
<xsl:template match="db:phrase[contains-token(@role, 'oxy_custom') and @oxy:type='oxy_content_highlight']">
  <xsl:call-template name="t:inline">
    <xsl:with-param name="extra-attributes" as="attribute()*">
      <xsl:attribute name="style" select="'color:rgb(' || @oxy:color || ');'"/>
    </xsl:with-param>
  </xsl:call-template>
</xsl:template>
...
<xsl:template match="db:phrase">
  <xsl:call-template name="t:inline">
    <xsl:with-param name="extra-attributes" as="attribute()*">
      <xsl:if test="@oxy:comment">
        <xsl:attribute name="title" select="string(@oxy:comment)"/>
      </xsl:if>
    </xsl:with-param>
  </xsl:call-template>
</xsl:template>
