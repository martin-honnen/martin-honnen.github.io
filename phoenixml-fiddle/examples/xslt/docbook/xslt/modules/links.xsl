...
<xsl:template match="db:anchor">
  <xsl:call-template name="t:inline">
    <xsl:with-param name="namemap" select="map{'*': 'a'}"/>
  </xsl:call-template>
</xsl:template>
...
<xsl:template match="db:link">
  <xsl:choose>
    ...
    <xsl:when test="@linkend or starts-with(@xlink:href, '#')">
    ...
    <xsl:otherwise>
      <xsl:variable name="content" as="item()*">
        <xsl:choose>
          ...
          <xsl:when test="$target/@xreflabel and not(f:is-true($prefer-xrefstyle))">
