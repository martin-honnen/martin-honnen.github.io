...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="id('div1')" mode="ixsl:onwheel">
    <xsl:sequence select="trace(saxon:timestamp() || ': ' || ixsl:event()?type)"/>
  </xsl:template>
  
  <xsl:template match="id('div2')" mode="ixsl:onwheel">
    <xsl:sequence select="trace(saxon:timestamp() || ': ' || ixsl:event()?type), ixsl:event() => ixsl:call('preventDefault', [])"/>
  </xsl:template>
