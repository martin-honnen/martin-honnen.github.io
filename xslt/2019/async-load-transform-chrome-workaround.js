      ...
      function transform(xmlUrl, xslUrl, xsltParams, targetElement) {
        Promise.all([loadDoc(xmlUrl), loadDoc(xslUrl)]).then(function(data) {
          ...
          if (typeof XSLTProcessor !== 'undefined') {
            ...
            var resultFrag = targetElement.ownerDocument.importNode(proc.transformToFragment(xmlDoc, targetElement.ownerDocument), true); // doesn't work with Webkit as script code in XSLT result fragment is not executed on appendChild: proc.transformToFragment(xmlDoc, targetElement.ownerDocument);
