  ...
  <xsl:template name="xsl:initial-template">
    <xsl:result-document href="#result">
		<section>
		  ...
		  <p><code>JSON.stringify</code> result: <code>{ixsl:window() => ixsl:get('JSON.stringify') => ixsl:apply([$object])}</code></p>
		  <xsl:message>$object: {ixsl:window() => ixsl:get('JSON.stringify') => ixsl:apply([$object])}</xsl:message>
		</section>
	</xsl:result-document>
  </xsl:template>
