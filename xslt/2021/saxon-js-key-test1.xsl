...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="xsl:initial-template">
    <xsl:result-document href="#content" method="ixsl:append-content">
      <p id="p{count(id('content', ixsl:page())/p) + 1}" class="foo">This is paragraph {count(id('content', ixsl:page())/p) + 1} created at {saxon:timestamp()}.</p>
    </xsl:result-document>
    <xsl:call-template name="check-content"/>
  </xsl:template>

  <xsl:template name="check-content">
    <xsl:message select="count(id('content', ixsl:page())/p), count(key('class', 'foo', ixsl:page()))"/>
  </xsl:template>
