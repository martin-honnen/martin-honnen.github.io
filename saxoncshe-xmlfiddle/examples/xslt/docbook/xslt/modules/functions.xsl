...
<xsl:function name="f:l10n-language" as="xs:string" cache="yes">
  ...
  <xsl:variable name="adjusted-language"
                select="if (contains($language, '_'))
                        then substring-before($language, '_')
                             || '-' || substring-after($language, '_')
                        else $language"/>
  ...
  <xsl:choose>
    ...
    <xsl:when test="doc-available(
                      resolve-uri(
                        substring-before($adjusted-language, '-')||'.xml', $v:localization-base-uri))">
      <xsl:sequence select="substring-before($adjusted-language, '-')"/>
    </xsl:when>
    <!-- or use the default -->
    <xsl:otherwise>
      <xsl:message>
