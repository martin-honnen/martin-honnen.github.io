...
<xsl:template match="db:imageobject">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      ...
      <!-- Construct the children in a sequence because we're going to 
           do a little post-processing -->
      <xsl:variable name="sources" as="element()*">
        <xsl:for-each select="$non-svg">
          <xsl:variable name="params" select="array:flatten(.?params)"/>
          <xsl:choose>
      ...
      <picture>
        ...
        <xsl:for-each-group select="$sources[self::h:source]" group-by="string(@media||@type)">
          <xsl:variable name="set" as="xs:string*">
            <xsl:for-each select="current-group()">
              <xsl:variable name="x" select="if (@pixeldensity) then ' '||@pixeldensity||'x' else ''"/>
              <xsl:sequence select="@srcset || $x"/>
            </xsl:for-each>
          </xsl:variable>

          <source>
            <xsl:sequence select="current-group()[1]/@media"/>
            <xsl:if test="current-group()/@type
                          and count(current-group()/@type) = count(current-group())
                          and count(distinct-values(current-group()/@type)) = 1">
              <xsl:sequence select="current-group()[1]/@type"/>
            </xsl:if>
            <xsl:attribute name="srcset" select="string-join($set, ', ')"/>
          </source>
        </xsl:for-each-group>
        <xsl:sequence select="$sources[self::h:img]"/>
      </picture>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:function name="f:object-properties" as="map(xs:string, xs:anyAtomicType)">
  ...
  <xsl:variable name="im-properties" as="map(xs:string, xs:anyAtomicType)">
    <xsl:sequence use-when="function-available('ext:image-metadata')"
                  select="ext:image-metadata($uri, $image-file)"/>
    <xsl:sequence use-when="not(function-available('ext:image-metadata'))"
                  select="map{}"/>
  </xsl:variable>

  <xsl:message use-when="$v:debug = 'image-properties'"
               select="'f:image-metadata:', serialize($im-properties, $v:as-json)"/>

  <xsl:variable name="ip-properties" as="map(xs:string, xs:anyAtomicType)">
    <xsl:sequence use-when="function-available('ext:image-properties')"
                  select="ext:image-properties($uri)"/>
    <xsl:sequence use-when="not(function-available('ext:image-properties'))"
                  select="map{}"/>
  </xsl:variable>

  <xsl:message use-when="$v:debug = 'image-properties'"
               select="'f:image-properties:', serialize($ip-properties, $v:as-json)"/>
  ...
  <xsl:variable name="properties" select="map:merge(($im-properties, $ip-properties))"/>

  <xsl:message use-when="$v:debug = 'image-properties'"
               select="'Merged:', serialize($properties, $v:as-json)"/>
