  ...
  try {
    if (inputType === 'XML') {
      transformationResult = SaxonJS.XPath.evaluate(
        "transform(map {'stylesheet-text': $stylesheet-text , 'source-node' : parse-xml($source-text), 'delivery-format' : 'serialized', 'stylesheet-params' : $xslt-params })",
        [],
        {
          params: {
            'stylesheet-text': xslt,
            'source-text': input,
            'xslt-params' : xsltParams
          }
        }
      );
    }
      ...
      transformationResult = SaxonJS.XPath.evaluate(
        "let $json-input := parse-json($json-input-string) return transform(map {'stylesheet-text': $stylesheet-text , 'global-context-item' : $json-input, 'initial-match-selection' : $json-input, 'delivery-format' : 'serialized', 'stylesheet-params' : $xslt-params })",
        [],
        {
          params: {
            'stylesheet-text': xslt,
      ...
      transformationResult = SaxonJS.XPath.evaluate(
        "transform(map {'stylesheet-text': $stylesheet-text , 'source-node' : ., 'delivery-format' : 'serialized', 'stylesheet-params' : $xslt-params })",
        htmlDoc,
        {
          params: {
            'stylesheet-text': xslt,
      ...
      transformationResult = SaxonJS.XPath.evaluate(
        "transform(map {'stylesheet-text': $stylesheet-text, 'delivery-format' : 'serialized', 'stylesheet-params' : $xslt-params })",
        [],
        {
          params: {
            'stylesheet-text': xslt,
